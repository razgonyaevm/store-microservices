...
Выполнено покрытие проекта автотестами. В системе реализовано сквозное интеграционное
тестирование всех ключевых бизнес-микросервисов с использованием **Testcontainers** и **встроенного брокера очередей**,
что гарантирует работоспособность и консистентность распределенной логики:

1. **Тестирование СУБД (PostgreSQL в `inventory-service` и `user-service`):**
    * Применяется механизм Spring Boot 3 **`@ServiceConnection`** для динамического подключения DataSource к
      запущенному в Docker тест-контейнеру PostgreSQL (`postgres:15-alpine`).
    * В `user-service` тесты проверяют полный цикл безопасности: регистрацию пользователя, хэширование паролей по
      алгоритму **BCrypt** в базе данных и корректность структуры сгенерированного JWT-токена.
2. **Тестирование NoSQL-кэша (Redis в `cart-service`):**
    * Используется динамический запуск инстанса Redis через **`GenericContainer`** (`redis:7-alpine`) со
      случайным маппингом портов через `@DynamicPropertySource`.
    * С помощью **`@MockBean`** изолируются внешние Feign-клиенты, что позволяет тестировать логику корзины без поднятия
      остальной инфраструктуры. Проверяются операции записи/чтения и JSON-сериализация объектов корзины.
3. **Тестирование транзакций и отправки событий (PostgreSQL + Kafka в `order-service`):**
    * Тест одновременно поднимает PostgreSQL (через Testcontainer) для проверки сохранения заказов и встроенный
      брокер **`@EmbeddedKafka`** (на порту 9092) для проверки отправки асинхронного события `OrderPlacedEvent`.
4. **Тестирование асинхронных слушателей (Kafka Consumer в `notification-service`):**
    * Используется встроенный брокер **`@EmbeddedKafka`** для симуляции отправки сообщений.
    * С помощью **`@SpyBean`** (Mockito) и библиотеки **Awaitility** настраивается асинхронное ожидание
      считывания события `@KafkaListener`-ом и проверяется успешность обработки лога.
5. **Анализ покрытия JaCoCo:**
    * Внедрен глобальный плагин `jacoco-maven-plugin`. При запуске команды `mvn clean verify` во всех модулях собирается
      статистика покрытия, которая автоматически агрегируется в модуле `coverage-report` в один единый интерактивный
      HTML-отчет.
